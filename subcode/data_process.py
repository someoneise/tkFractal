...
def generateDravesIFS(
    ...
    :param probabilities: A 1D numpy array of the probs. for the transf.
    :param iterations: The number of iterations to perform.
    :return: A tuple of 3 numpy arrays containing the x and y coordinates
    ...
    for i in range(iterations):
        ...
        function_index = weighted_rand_choice(probabilities)

        # Apply the transformation
        xy = np.dot(coeffs[function_index], xy) + sums[function_index].T

        # Change the color according to Drave's paper
        c = (c + ci[function_index]) / 2
...
def parallel_generateDravesIFS(coeffs:NDarray,
sums:NDarray,
probabilities:NDarray,
iterations:int=50000,
num_processes:int=4) -> tuple:
    # Create a multiprocessing Pool
    with Pool(num_processes) as pool:
        # Run generateDravesIFS in parallel and collect the results
        results = pool.starmap(generateDravesIFS,
        [(coeffs,
        sums,
        probabilities,
        iterations) for _ in range(num_processes)])
